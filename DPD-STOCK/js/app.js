// Authentic user database
    ...
    const i18n = {
      ...
      "จำนวนที่นำเข้า": "Quantity to Receive",
    ...
    function loadDatabase() {
      ...
      // Robust products loading
      if (storedProducts) {
        try {
          const parsed = JSON.parse(storedProducts);
          if (Array.isArray(parsed)) {
            products = parsed.filter(p => p && typeof p === 'object' && p.code && p.name);
            products.forEach(p => {
              p.qty = typeof p.qty === 'number' && !isNaN(p.qty) ? p.qty : 0;
              p.min = typeof p.min === 'number' && !isNaN(p.min) ? p.min : 0;
              p.unit = p.unit || 'ชิ้น';
              p.cat = p.cat || 'อื่นๆ';
              p.loc = p.loc || '';
            });
      ...
      if (storedHistory) {
        try {
          ...
          if (Array.isArray(parsed)) {
            history = parsed.filter(h => h && typeof h === 'object' && h.code && h.name);
            history.forEach(h => {
              h.date = h.date || new Date().toISOString().slice(0, 10);
              h.type = h.type || 'ปรับปรุง';
              h.qty = typeof h.qty === 'number' && !isNaN(h.qty) ? h.qty : 0;
              h.user = h.user || 'system';
              h.note = h.note || '';
            });
          } else { history = fallbackHistory; }
        } catch (e) { history = fallbackHistory; }
      } else { history = fallbackHistory; }

      // Robust personnel loading
      if (storedPersonnel) {
        try {
          const parsed = JSON.parse(storedPersonnel);
          if (Array.isArray(parsed)) {
            PERSONNEL = parsed.filter(p => p && typeof p === 'object' && p.name);
            PERSONNEL.forEach(p => {
              p.position = p.position || 'กกม.บก.ซอ.';
              p.phone = p.phone || '';
            });
          } else { PERSONNEL = DEFAULT_PERSONNEL; }
        } catch (e) { PERSONNEL = DEFAULT_PERSONNEL; }
      } else { PERSONNEL = DEFAULT_PERSONNEL; }
    ...
    function doReceive() {
      ...
      if (isNaN(qty) || qty <= 0) {
        showToast('กรุณาระบุจำนวนที่นำเข้าให้ถูกต้อง', 'danger');
        return;
      }
      ...
      if (p) {
        ...
        const combinedNote = details.join(' | ') || 'นำเข้าพัสดุ';
        ...
        renderStock();
        closeRecvModal();
        showToast('บันทึกรับสินค้าเข้าคลังสำเร็จ', 'success');
      }
    }
    ...
    function addProduct() {
      ...
      history.unshift({
        ...
        qty,
        user: currentUser || 'admin',
        note: 'จดทะเบียนพัสดุรายการใหม่เข้าสต็อกคลังสินค้า'
      });
      saveDatabase();
